 ...
 */
public class FragmentVideoPage extends FragmentBase implements View.OnClickListener {
    private View mView;
        ...
        worker.schedule(new Action0() {
            ...
            public void call() {
                try {
                    ...
                    Object asObject = aCache.getAsObject(Values.AcacheKey.KEY_VIDEO_TYPELIST);
                    if (asObject != null && asObject instanceof ArrayList) {
                        oriList = (ArrayList<TypeBean>) asObject;
                    }

                    if (oriList == null || oriList.size() == 0) {
                        InputStream open = mActivity.getAssets().open("default_videopage.json");
                    ...
                    LogHelper.d("fragvideopage", "loadData success default---");
                } catch (Exception e) {
                    LogHelper.d("fragvideopage", "loadData default 没取到---");
                    e.printStackTrace();
                }
                worker.unsubscribe();
            }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.edit_channel:
                ...
                mActivity.startActivityForResult(i, 103);
        ...
        if (requestCode == 103 && resultCode == Activity.RESULT_OK) {
            ...
            if (list != null && list.size() > 0) {
                ...
                worker.schedule(new Action0() {
                    ...
                    public void call() {
                        ACache aCache = ACache.get(mActivity);
                        aCache.put(Values.AcacheKey.KEY_VIDEO_TYPELIST, mDataList);
                        worker.unsubscribe();
                    }
                });
            }
